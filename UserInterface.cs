...
namespace DRnamespace
{
    public partial class MainWindow : Window
    {
            ...
            while (true)
            {
                Action dele = delegate ()
                {
                    ...
                    if (System.Windows.Forms.Control.MouseButtons == System.Windows.Forms.MouseButtons.Left
                        && System.Windows.Forms.Control.MousePosition != MouseP)
                    {
                        //DragMove();
                    }
        ...
        void NewDragMove()
        {
            while (true)
            {
                if (Draging && System.Windows.Forms.Control.MouseButtons != System.Windows.Forms.MouseButtons.Left)
                {
                    Draging = false; 
                ...
                else if (Draging)
                {
                    Dispatcher.Invoke(new Action(() =>
                    {
                        Left = -relativePosition.X + System.Windows.Forms.Control.MousePosition.X;
                        Top = -relativePosition.Y + System.Windows.Forms.Control.MousePosition.Y;
                    }));
                }

                SpinWait.SpinUntil(() => false, Draging ? 10 : 100);
            }
        }
        ...
        {
            if (!Draging && System.Windows.Forms.Control.MouseButtons != System.Windows.Forms.MouseButtons.None)
            {
                if ((e.GetPosition(this) - relativePosition).Length > DragThreshold)
                {
                    IsEnabled = false;
                    Draging = true;
                }
        ...
        private void FileNameBox_TextChanged(object sender, System.Windows.Controls.TextChangedEventArgs e)
        {
            if (FileNameBox != null)
            {
                ...
                {
                    int index = FileNameBox.Text.LastIndexOf('_');
                    if (index > 0)
                    {
                        FileNameBox.Text = FileNameBox.Text.Substring(0, index);
                    }
        ...
        private Boolean isParametersReady()
        {
            if (FileNameBox.Text.Length > 0 && FrameRateBox.Text.Length > 0 && SpeedBox.Text.Length > 0)
            {
                try
                {
                    ...
                    Convert.ToDouble(SpeedBox.Text);
                }
                catch
                {
                    return false;
